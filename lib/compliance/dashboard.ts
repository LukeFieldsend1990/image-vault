...
// A certificate row enriched for display: the subject it covers (production /
// talent / organisation / licence name) and who generated it, so a compliance
// account can see at a glance which cert it minted and for whom.
export interface RecentCertificate {
  id: string;
  scope: string;
  scopeId: string;
  subjectName: string | null;   // resolved name of the production/talent/org/licence
  generatedAt: number;
  generatedById: string;        // user id of the generator (client marks "you")
  generatedByName: string | null; // generator's email
}
...
//
// Raw certificate rows only carry a scope + scopeId + generator id. Resolve those
// into human-readable subject names and the generator's email (batched, no N+1) so
// the "Recent certificates" list reads e.g. "Production certificate · Avatar 3,
// generated by you" rather than a bare "Production certificate".

type RawCertRow = { id: string; scope: string; scopeId: string; generatedAt: number; generatedBy: string };

async function enrichCertificates(db: Db, rows: RawCertRow[]): Promise<RecentCertificate[]> {
  if (rows.length === 0) return [];

  const idsForScope = (scope: string) => [...new Set(rows.filter((r) => r.scope === scope).map((r) => r.scopeId))];
  const prodIds = idsForScope("production");
  const talentIds = idsForScope("talent");
  const orgIds = idsForScope("organisation");
  const licIds = idsForScope("licence");
  const generatorIds = [...new Set(rows.map((r) => r.generatedBy))];

  const subjectName = new Map<string, string>(); // `${scope}:${scopeId}` → name
  const put = (scope: string, id: string, name: string | null) => {
    if (name) subjectName.set(`${scope}:${id}`, name);
  };

  if (prodIds.length) {
    for (const r of await db.select({ id: productions.id, name: productions.name }).from(productions).where(inArray(productions.id, prodIds)).all()) {
      put("production", r.id, r.name);
    }
  }
  if (orgIds.length) {
    for (const r of await db.select({ id: organisations.id, name: organisations.name }).from(organisations).where(inArray(organisations.id, orgIds)).all()) {
      put("organisation", r.id, r.name);
    }
  }
  if (licIds.length) {
    for (const r of await db.select({ id: licences.id, name: licences.projectName }).from(licences).where(inArray(licences.id, licIds)).all()) {
      put("licence", r.id, r.name);
    }
  }
  if (talentIds.length) {
    // Prefer the talent's full name; fall back to their account email.
    for (const r of await db.select({ id: talentProfiles.userId, name: talentProfiles.fullName }).from(talentProfiles).where(inArray(talentProfiles.userId, talentIds)).all()) {
      put("talent", r.id, r.name);
    }
    for (const r of await db.select({ id: users.id, email: users.email }).from(users).where(inArray(users.id, talentIds)).all()) {
      if (!subjectName.has(`talent:${r.id}`)) put("talent", r.id, r.email);
    }
  }

  const generatorEmail = new Map<string, string>();
  if (generatorIds.length) {
    for (const r of await db.select({ id: users.id, email: users.email }).from(users).where(inArray(users.id, generatorIds)).all()) {
      generatorEmail.set(r.id, r.email);
    }
  }
  ...
  return rows.map((r) => ({
    ...
    scope: r.scope,
    scopeId: r.scopeId,
    subjectName: subjectName.get(`${r.scope}:${r.scopeId}`) ?? null,
    generatedAt: r.generatedAt,
    generatedById: r.generatedBy,
    generatedByName: generatorEmail.get(r.generatedBy) ?? null,
  }));
}
...
export async function buildOrgDashboard(
  ...
  const certRows = await db
    .select({
      ...
      scopeId: complianceCertificates.scopeId,
      generatedAt: complianceCertificates.generatedAt,
      generatedBy: complianceCertificates.generatedBy,
    })
...
export async function buildTalentDashboard(
  ...
  const certRows = await db
    .select({ id: complianceCertificates.id, scope: complianceCertificates.scope, scopeId: complianceCertificates.scopeId, generatedAt: complianceCertificates.generatedAt, generatedBy: complianceCertificates.generatedBy })
    .from(complianceCertificates)
