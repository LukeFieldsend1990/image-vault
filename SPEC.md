...
**Cost reduction (2026-08-22).** Apify's fee buys the anti-bot infrastructure for Instagram/TikTok — the one part not worth rebuilding in-house (residential proxies plus a permanent maintenance treadmill for marginal net savings). Two levers were built instead:

- **Runtime-swappable actors** (`lib/monitor/ingest/actor-settings.ts`): the hashtag/search/profile/TikTok actor ids and Instagram results-per-query live in `ai_settings` (`apify_hashtag_actor`, `apify_search_actor`, `apify_profile_actor`, `apify_tiktok_actor`, `apify_results_per_query`), editable from `/admin/monitor` — the follows-actor precedent generalised. The compiled defaults keep today's exact input shapes (pinned in `__tests__/lib/monitor/actor-swap.test.ts`); an overridden actor is sent a superset input with common alias keys, and billing stays bounded by the `maxItems` URL param Apify itself enforces. Promote a cheaper pay-per-result actor only after `scripts/discovery-bakeoff.mjs --ig-hashtag-actor=<id> --tiktok-actor=<id>` shows comparable yield and thumbnail coverage (no thumbnail = face/synthetic layers starve).
- **Free official APIs for the side surfaces**: with `REDDIT_CLIENT_ID`/`REDDIT_CLIENT_SECRET` set, reddit runs on Reddit's OAuth client-credentials API (`lib/monitor/ingest/reddit-api.ts`); with `BRAVE_SEARCH_API_KEY` set, google/getty run on Brave Search (`lib/monitor/ingest/brave-serp.ts`, free tier 2,000 queries/month, counter in `ai_settings.brave_search_month`/`brave_search_count`). Both reuse the incumbent modules' query builders and candidate mappers, log per-term zero-cost query rows (the YouTube precedent — no `apify_usage` rows), run **outside** the Apify ceiling, and fall back to the Apify actors when their credentials are absent. Note the ordering consequence: these surfaces now survive a spent Apify ceiling.
