...
export default function DeleteCompanyButton({ id, name }: { id: string; name: string }) {
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    if (
      !window.confirm(
        `Delete "${name}"?\n\nThis removes the company. Any linked productions and licences will be detached but kept. This cannot be undone.`
      )
    ) {
      return;
    }
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/organisations/${id}`, { method: "DELETE" });
      if (!res.ok) {
