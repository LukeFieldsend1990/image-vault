 ...
 * SALMON — lifted brick (brick is too dark on dark). It is the only dark chrome the brand
 ...
 *
 * Compose a statement with <em> for the salmon emphasis, e.g.
...
export default function Inlay({
  ...
  eyebrow?: ReactNode;
  /** The statement. Wrap the emphasised clause in <em> for the salmon italic. */
  children: ReactNode;
  ...
  return (
    ...
    >
          ...
          >
            {gate && (
              <span className="flex items-stretch" style={{ gap: "3px", height: "14px" }} aria-hidden>
                <span style={{ width: "3px", borderRadius: "1.5px", background: "var(--color-salmon)" }} />
                <span style={{ width: "3px", borderRadius: "1.5px", background: "var(--color-salmon)" }} />
              </span>
            )}
            {footnote}
      ...
      {/* salmon emphasis for any <em> inside the statement */}
      <style>{`.inlay-statement em { font-style: italic; color: var(--color-salmon); }`}</style>
    </div>
  );
}
